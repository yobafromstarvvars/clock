...
class MainApplication:
    ...
    def __init__(self, master):
        ...
        self.master.config(padx=60, pady=50, bg=BACKGROUND_COLOUR)

        self.loginLabel = tk.Label(self.master, text="Clock", bg=BACKGROUND_COLOUR, fg=FOREGROUND_COLOUR, pady=20, font=self.large_font)
        self.loginLabel.grid(row=0, column=0, columnspan=2)

        self.userLabel = tk.Label(self.master, text="User:", bg=BACKGROUND_COLOUR, fg=FOREGROUND_COLOUR)
        self.userLabel.grid(row=1, column=0, pady=5)
        self.userLabel.config(justify="right")

        self.userEntry = tk.Entry(self.master, textvariable=self.user, width=30)
        self.userEntry.grid(row=1, column=1, pady=5)
        self.userEntry.config(fg="black", justify="left")

        self.passwordLabel = tk.Label(self.master, text="Password:", bg=BACKGROUND_COLOUR, fg=FOREGROUND_COLOUR)
        self.passwordLabel.grid(row=2, column=0, pady=5)
        self.passwordLabel.config(justify="right")

        self.passwordEntry = tk.Entry(self.master, textvariable=self.password, width=30)
        self.passwordEntry.grid(row=2, column=1, pady=5)
        self.passwordEntry.config(show="*", justify="left")

        self.bottonLog = tk.Button(self.master, fg=BACKGROUND_COLOUR, text="------>", cursor="hand2", font=self.large_font, command=lambda:self.log_in())
        self.bottonLog.grid(row=3, column=0, columnspan=2, padx=10, pady=11)
